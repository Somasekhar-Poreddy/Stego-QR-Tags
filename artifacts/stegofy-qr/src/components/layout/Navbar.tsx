    ...
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 px-4">
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-2 flex-shrink-0">
          ...
          <button
            className="p-2 text-foreground/60 hover:text-primary transition-colors rounded-lg hover:bg-slate-100 relative"
            aria-label="Cart"
          >
        ...
        <div className="md:hidden flex items-center gap-1.5">
          <a href="#" className="text-xs font-medium text-foreground/70 hover:text-primary transition-colors px-2 py-1.5">
            Login
          </a>
          <a href="#products" className="text-xs font-semibold bg-primary text-white px-3 py-1.5 rounded-lg hover:bg-primary/90 transition-colors whitespace-nowrap">
            Sign Up
          </a>
          <button
            className="p-2 text-foreground/60 hover:text-primary transition-colors rounded-lg hover:bg-slate-100 relative"
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="px-4 py-5 flex flex-col gap-1">
              ...
              <a href="#faqs" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium px-3 py-2.5 hover:bg-slate-50 rounded-xl">FAQs</a>
              <a href="#partner" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium px-3 py-2.5 hover:bg-slate-50 rounded-xl">Partner With Us</a>

              <div className="h-px bg-gray-100 my-1" />
              <p className="text-xs text-muted-foreground px-3 pt-1 font-semibold uppercase tracking-wider">About Us</p>
              {ABOUT_MENU.map((item) => (
                <a key={item.name} href={item.href} onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium px-3 py-2.5 hover:bg-slate-50 rounded-xl flex items-center gap-2">
                  <item.icon className="w-4 h-4 text-primary" />
                  {item.name}
                </a>
              ))}

              <a href="#contact" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium px-3 py-2.5 hover:bg-slate-50 rounded-xl">Contact Us</a>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
